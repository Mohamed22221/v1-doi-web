...
interface SellerRejectedContentProps {
  locale: Locale;
}

/**
 * SellerRejectedContent
 *
 * Displays the rejected verification screen for sellers.
 * Includes the same illustration as the pending screen but with updated messaging.
 *
 * Note: This component is a Server Component and uses 'use cache'.
 */
export default async function SellerRejectedContent({ locale }: SellerRejectedContentProps) {
  "use cache";
  cacheLife("days");
    ...
    <section className="flex flex-col items-center gap-3 text-center md:gap-4">
      ...
      <div className="relative aspect-square max-h-[325px] w-full max-w-[325px]">
        <Image
          ...
          alt={t("seller-rejected.illustrationAlt")}
          fill
      ...
      {/* Message */}
      <HeaderSidebar
        title={t("seller-rejected.title")}
        subtitle={t("seller-rejected.description")}
        as="h2"
