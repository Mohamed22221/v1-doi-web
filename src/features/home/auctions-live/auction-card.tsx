...
export function AuctionCard({ title, timeLabel, imageUrl, orderNumber }: AuctionCardProps) {
  return (
    <div className="relative flex h-[320px] w-[274px] shrink-0 flex-col overflow-hidden rounded-[28px] lg:h-[422px] lg:w-[274px]">
      {/* Background image */}
      ...
      <div className="absolute end-4 top-4 z-20 flex size-10 items-center justify-center rounded-full bg-white/20">
        <span className="font-[Montserrat,sans-serif] text-base font-medium text-primary-50">
          {orderNumber}
        </span>
      </div>

      {/* Bottom content overlay */}
      <div className="absolute inset-x-3.5 bottom-3.5 z-20 flex flex-col items-start gap-2 rounded-2xl bg-white/12 p-4">
        {/* Title */}
        <h3 className="line-clamp-1 w-full text-start text-h5 leading-snug font-bold tracking-wide text-white">
          {title}
        </h3>
        {/* Time row: dot + time text, right-aligned */}
        <div className="flex items-center gap-2">
          <span className="size-2 rounded-[4px] bg-primary-400" aria-hidden="true" />
